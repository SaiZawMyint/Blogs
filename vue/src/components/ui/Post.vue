<template>
    <div class="w-full flex justify-center relative">
        <div class="flex flex-col p-2 w-[30%] max-w-[600px] h-full fixed left-0">
        ...
        <div class="flex flex-col p-2 mx-2 my-4 w-[40%] rounded border-2" v-html="blogsTemplate"></div>
        <div class="flex flex-col p-2 text-sm w-[28%] rounded border-2 my-4 mx-2 fixed fixed-rl-box right-0">
            <div class="w-full h-full overflow-y-auto overflow-x-hidden p-0">
                ...
                <div class="flex items-center justify-center h-[400px] max-h-[80%]" v-if="data.data.comment.length == 0">
...
const blogsTemplate = ref('<i class="text-sm text-gray-300 mt-10">Loading blogs</i>')
const inputData = ref({
    comment: ''
})
store.dispatch('getBlogById',id).then((res)=>{
    ...
    blogsTemplate.value = (itech().cms().previewTemplate(data.value.postData))
})
