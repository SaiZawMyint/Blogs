<template>
    <div class="w-full flex justify-center relative">
        <div class="flex pr-2 flex-col w-[25%] max-w-[600px]  fixed-rl-box fixed left-0">
            <div class="bg-gray-100 rounded-lg shadow flex flex-col justify-between h-full">
                <div class="h-full flex flex-col">
                    ...
                    <div class="w-full h-full overflow-y-auto">
                        <div class="mb-2">
                            <img :src="data.postData.cover" alt="">
                            <div class="w-full p-2 bg-gray-200/30 backdrop-blur p-2">
                                {{data.description}}
                            </div>
                        </div>
                        <div class="w-full p-2">
                            ...
                            <a v-for="outline in data.outlines" :href="`#${generateOutlineId(outline)}`"
                                ...
                                <span class="px-2">{{outline.name}}</span>
                            </a>
                        </div>
                    </div>
                </div>
        ...
        <div class="flex flex-col p-2 mx-2 w-[50%] rounded border-2" v-html="blogsTemplate"></div>
        <div class="flex flex-col p-2 text-sm w-[24%] rounded border-2 mx-2 fixed fixed-rl-box right-0">
...
store.dispatch('getBlogById',id).then((res)=>{
    ...
    console.log(data.value.outlines)
    blogsTemplate.value = data.value.postData.post.length == 0 ? `<i class="text-sm text-gray-300 my-10 text-center w-full">No template created! </i>`: (itech().cms().previewTemplate(data.value.postData.post))
})
